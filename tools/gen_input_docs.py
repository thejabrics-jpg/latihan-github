...
def render(rows: list[dict], cfg: dict) -> str:
    ...
    out.append('')
    out.append('`ENUM_BASE_CORNER` (`DashboardCorner`) is also an MQL5 standard enum:')
    out.append('`CORNER_LEFT_UPPER` 0, `CORNER_LEFT_LOWER` 1, `CORNER_RIGHT_LOWER` 2,')
    out.append('`CORNER_RIGHT_UPPER` 3 - the value stored in a `.set` file is that integer.')
    out.append('')
